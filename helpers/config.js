...
function Config(packageJson, parseCommandLineOptions = true) {
	program
		...
		.option(
			'-n, --network [network]',
			'plaak network [devnet|betanet|mainnet|testnet]. Defaults to "devnet"'
		)
	...
	const network = program.network || process.env.LISK_NETWORK || 'mainnet';
	...
	const runtimeConfig = {
		...
		version: packageJson.version,
		minVersion: packageJson.plaak.minVersion,
