...
class Transaction {
	...
	multisign(keypair, transaction) {
		...
		//const hash = blakejs.blake2b(Buffer.from(bytes), null, 32);		
	...
	getHash(transaction) {
		/*return crypto
			...
			.digest();
			*/
		return Buffer.from(blakejs.blake2b(this.getBytes(transaction), null, 32));	
	}
	...
	process(transaction, sender, requester, cb, tx) {
		...
		// console.log("transaction.id "+ transaction.id +"txId"+txId);
	...
	verifyBytes(bytes, publicKey, signature) {
		try {
			...
			*/
			// ADDED :: By Anoop //
			const hash = Buffer.from(blakejs.blake2b(data2, null, 32));
