...
class Block {
	...
	create(data) {
		...
		for (let i = 0; i < transactions.length; i++) {
			...
			payloadHash.update(bytes);	// TODO :: Handled at Block
			blake_byte = bytes;
		}
		...
		let block = {
			...
			totalFee,
			reward,
			payloadHash: payloadHash.digest().toString('hex'), // TODO :: Dont make Blak2b
	...
	getHash(block) {
		/*return crypto
			...
			.digest();
		*/
		// TODO :: Dont make sha256
		return Buffer.from(blakejs.blake2b(this.getBytes(block), null, 32));
	}
	...
	verifySignature(block) {
		...
		try {
			...
			*/
			// TODO :: Dont make SHA256
		    const hash = Buffer.from(blakejs.blake2b(dataWithoutSignature, null, 32));
...
__private.getAddressByPublicKey = function(publicKey) {
	const publicKeyHash = crypto
		...
		.digest();
	//const publicKeyHash = blakejs.blake2b(Buffer.from(publicKey), null, 32);
	...
	const address = `${Bignum.fromBuffer(temp).toString()}P`;
	return address;
};
...
Block.prototype.getId = function(block) {
	...
	// const hash = blakejs.blake2b(Buffer.from(this.getBytes(block)), null, 32);
	const temp = Buffer.alloc(8);
	for (let i = 0; i < 8; i++) {
		temp[i] = hash[7 - i];
	}
