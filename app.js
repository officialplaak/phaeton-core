...
// Define plaak network env variable to be used by child processes load config files
process.env.PHAETON_NETWORK = appConfig.network;
...
logger.info(`Starting Plaak with "${appConfig.network}" genesis block.`);
// Run domain
d.run(() => {
	var modules = [];
	async.auto(
			...
			listenHttp: [
				...
				function(scope, cb) {
					scope.network.server.listen(
						...
						err => {
							scope.logger.info(
								`Plaak started: ${scope.config.address}:${scope.config.httpPort}`
							);
