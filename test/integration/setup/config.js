...
module.exports = {
	generatePhaetonConfigs(BROADCASTING = true, TOTAL_PEERS = 10) {
		...
		const configurations = _.range(TOTAL_PEERS).map(index => {
			...
			devConfigCopy.logFileName = `../logs/phaeton_node_${index}.log`;
		...
		const pm2Config = configurations.reduce(
			(pm2Config, configuration) => {
				...
				pm2Config.apps.push({
					...
					error_file: `./test/integration/logs/phaeton-test-node-${index}.err.log`,
					out_file: `./test/integration/logs/phaeton-test-node-${index}.out.log`,
				});
				return pm2Config;
			},
			{ apps: [] }
