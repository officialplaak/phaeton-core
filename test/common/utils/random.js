...
random.account = function() {
	...
	account.publicKey = phaeton.cryptography.getKeys(account.passphrase).publicKey;
	account.address = phaeton.cryptography.getAddressFromPublicKey(
		account.publicKey
	);
	account.secondPublicKey = phaeton.cryptography.getKeys(
...
random.transaction = function(offset) {
	return phaeton.transaction.transfer({
		amount: 1,
