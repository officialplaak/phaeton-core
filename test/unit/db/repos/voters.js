...
describe('db', () => {
	before(done => {
		dbSandbox = new DBSandbox(__testContext.config.db, 'phaeton_test_db_voters');
	...
	describe('VotersRepository', () => {
		...
		describe('count()', () => {
			...
			it('should return integer type count of total votes of a specific phaeton id', function*() {
				const delegate = accountsFixtures.Delegate();
				const account1 = accountsFixtures.Account();
				const account2 = accountsFixtures.Account();
				yield db.accounts.insert(delegate);
