...
describe('transaction', () => {
	...
	before(done => {
		...
		application.init(
			{ sandbox: { name: 'phaeton_test_logic_transactions' } },
			(err, scope) => {
	...
	describe('getBytes', () => {
		...
		it('should return same result of getBytes using /logic/transaction and phaeton-validator package (without data field)', () => {
			var transactionBytesFromLogic = transactionLogic.getBytes(
				validTransaction
			);
			var transactionBytesFromPhaetonJs = phaeton.transaction.utils.getTransactionBytes(
				validTransaction
			);

			return expect(
				transactionBytesFromLogic.equals(transactionBytesFromPhaetonJs)
			).to.be.ok;
		});
	...
	describe('verify', () => {
		function createAndProcess(transactionData, sender, cb) {
			...
			var transaction = phaeton.transaction.transfer(transferObject);
