...
describe('POST /api/transactions (type 3) votes', () => {
	...
	*/
	...
	before(() => {
		var transactions = [];
		var transaction1 = phaeton.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: delegateAccount.address,
		});
		var transaction2 = phaeton.transaction.transfer({
			amount: constants.fees.vote,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: accountMinimalFunds.address,
		});
		var transaction3 = phaeton.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: accountFixtures.existingDelegate.address,
		});
		var transaction4 = phaeton.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: accountMaxVotesPerTransaction.address,
		});
		var transaction5 = phaeton.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
		...
		return Promise.all(promises)
			...
			.then(() => {
				...
				var transaction = phaeton.transaction.registerDelegate({
	...
	describe('transactions processing', () => {
		it('using invalid publicKey should fail', () => {
			transaction = phaeton.transaction.castVotes({
				passphrase: delegateAccount.passphrase,
		...
		it(`upvoting ${
			...
			var transaction1 = phaeton.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
				votes: delegatesMaxVotesPerAccount.slice(0, 33).map(delegate => {
					return `${delegate.publicKey}`;
				}),
			});
			var transaction2 = phaeton.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
				votes: delegatesMaxVotesPerAccount.slice(33, 66).map(delegate => {
					return `${delegate.publicKey}`;
				}),
			});
			var transaction3 = phaeton.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
				votes: delegatesMaxVotesPerAccount.slice(66, 99).map(delegate => {
					return `${delegate.publicKey}`;
				}),
			});
			var transaction4 = phaeton.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
