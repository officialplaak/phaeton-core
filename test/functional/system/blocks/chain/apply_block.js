...
describe('system test (blocks) - chain/applyBlock', () => {
	...
	beforeEach('send funds to accounts', done => {
		...
		const fundTrsForAccount1 = phaeton.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: blockAccount1.address,
		});

		const fundTrsForAccount2 = phaeton.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: blockAccount2.address,
		});

		const fundTrsForAccount3 = phaeton.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: poolAccount3.address,
		});

		const fundTrsForAccount4 = phaeton.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
	...
	describe('applyBlock', () => {
		...
		beforeEach('create block', done => {
			blockTransaction1 = phaeton.transaction.registerDelegate({
				passphrase: blockAccount1.passphrase,
				username: blockAccount1.username,
			});
			blockTransaction2 = phaeton.transaction.registerDelegate({
		...
		describe('undoUnconfirmedList', () => {
			...
			beforeEach('with transactions in unconfirmed queue', done => {
				transaction3 = phaeton.transaction.registerSecondPassphrase({
					passphrase: poolAccount3.passphrase,
					secondPassphrase: poolAccount3.secondPassphrase,
				});

				transaction4 = phaeton.transaction.registerSecondPassphrase({
