...
describe('system test (type 4) - effect of multisignature registration on memory tables', () => {
	...
	describe('forge block with multisignature transaction', () => {
		before('forge block with multisignature transaction', done => {
			...
			multisigTransaction = phaeton.transaction.registerMultisignature({
				passphrase: multisigAccount.passphrase,
				keysgroup,
				lifetime: 4,
				minimum: 2,
			});
			var sign1 = phaeton.transaction.utils.multiSignTransaction(
				multisigTransaction,
				signer1.passphrase
			);
			var sign2 = phaeton.transaction.utils.multiSignTransaction(
				multisigTransaction,
				signer2.passphrase
			);
	...
	describe('apply unconfirmed transaction', () => {
		...
		describe('with another multisig transaction', () => {
			...
			before('process multisignature transaction', done => {
				...
				multisigTransaction2 = phaeton.transaction.registerMultisignature({
					passphrase: multisigAccount.passphrase,
					keysgroup,
					lifetime: 4,
					minimum: 2,
				...
				var sign3 = phaeton.transaction.utils.multiSignTransaction(
					multisigTransaction2,
					signer3.passphrase
				);
				var sign4 = phaeton.transaction.utils.multiSignTransaction(
					multisigTransaction2,
					signer4.passphrase
				);
