...
describe('inert transactions', () => {
	...
	const transferInertTransaction = phaeton.transaction.transfer({
		recipientId: recipientAccount.address,
		amount: 1000000000 * 100,
		passphrase: senderAccount.passphrase,
	});

	const voteInertTransaction = phaeton.transaction.castVotes({
		passphrase: recipientAccount.passphrase,
		votes: [`${accountFixtures.existingDelegate.publicKey}`],
	});

	const delegateInertTransaction = phaeton.transaction.registerDelegate({
		passphrase: recipientAccount.passphrase,
	...
	describe('send funds to account', () => {
		before(done => {
			const transferTransaction = phaeton.transaction.transfer({
				recipientId: recipientAccount.address,
