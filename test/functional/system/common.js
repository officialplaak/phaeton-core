...
function beforeBlock(type, cb) {
	...
	before(
		'init sandboxed application, credit account and register dapp',
			...
			application.init(
				{ sandbox: { name: `phaeton_test_${type}` } },
				(err, library) => {
					cb(library);
					done();
				}
...
function loadTransactionType(key, account, dapp, secondPassphrase, cb) {
	...
	switch (key) {
		case 'SEND':
			transaction = phaeton.transaction.transfer({
				amount: 1,
			...
			break;
		case 'SIGNATURE':
			transaction = phaeton.transaction.registerSecondPassphrase({
				passphrase: account.passphrase,
				secondPassphrase: account.secondPassphrase,
			});
			break;
		case 'DELEGATE':
			transaction = phaeton.transaction.registerDelegate({
				passphrase: accountCopy.passphrase,
				secondPassphrase: accountCopy.secondPassphrase,
		...
		case 'VOTE':
			...
			break;
		case 'MULTI':
			transaction = phaeton.transaction.registerMultisignature({
				passphrase: accountCopy.passphrase,
				secondPassphrase: accountCopy.secondPassphrase,
			...
			break;
		case 'DAPP':
			transaction = phaeton.transaction.createDapp({
				passphrase: accountCopy.passphrase,
				secondPassphrase: accountCopy.secondPassphrase,
