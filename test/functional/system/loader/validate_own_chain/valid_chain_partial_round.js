...
describe('validateOwnChain', () => {
	...
	localCommon.beforeBlock(
		'phaeton_functional_validate_own_chain_valid_chain_partial_round',
		lib => {
			library = lib;
	...
	describe('forge 150 blocks with version = 0', () => {
		before(() => {
			...
			return Promise.mapSeries([...Array(150 - 1)], () => {
				return addTransactionsAndForgePromise(library, [], 0);
			});
		});

		it('blockchain should be at height 150', () => {
		...
		describe('increase block version = 1 and exceptions for height = 150', () => {
			...
			describe('forge 5 more blocks', () => {
				...
				it('blockchain should be at height 155', () => {
					const lastBlock = library.modules.blocks.lastBlock.get();
