...
__private.decryptPassphrase = function(encryptedPassphrase, password) {
	return phaeton.cryptography.decryptPassphraseWithPassword(
		phaeton.cryptography.parseEncryptedPassphrase(encryptedPassphrase),
		password
	);
};
...
__private.loadDelegates = function(cb) {
	...
	async.eachSeries(
		...
		(encryptedItem, seriesCb) => {
			...
			// const keypair = library.ed.makeKeypair(blakejs.blake2b(Buffer.from(passphrase,'utf8'), null, 32));
...
Delegates.prototype.updateForgingStatus = function(
	...
	if (encryptedItem) {
		...
		// keypair = library.ed.makeKeypair(blakejs.blake2b(Buffer.from(passphrase,'utf8'), null, 32));
...
Delegates.prototype.generateDelegateList = function(round, source, cb, tx) {
	...
	source((err, truncDelegateList) => {
		...
		//  let currentSeed = blakejs.blake2b(Buffer.from(seedSource,'utf8'), null, 32);
		...
		for (let i = 0, delCount = truncDelegateList.length; i < delCount; i++) {
			...
			// currentSeed = blakejs.blake2b(Buffer.from(currentSeed), null, 32);	
		}
